...
exports.tree2array = function t2a(path, value, nodes) {
  ...
  if (my_nodes && my_nodes.length > 0) {
    ...
    my_nodes.forEach(function (node, i) {
      my_array[i + 1] = [node, t2a("".concat(path || "").concat(separator).concat(node), value, nodes, separator = separator)];
    });
    return my_array;
  } else {
    return [value(path)];
...
exports.tree2object = function t2o(path, value, nodes) {
  ...
  if (my_nodes && my_nodes.length > 0) {
    ...
    my_nodes.forEach(function (node) {
      my_object[node] = t2o("".concat(path || "").concat(separator).concat(node), value, nodes, separator = separator, value_key = value_key);
    });
    return my_object;
  } else {
    return my_value;
